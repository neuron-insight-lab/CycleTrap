...
def get_loss(outputs, output_len, eos_token_id):
    ...
    return {"loss1": loss_uncertainty, "loss2": loss_eos}
...
def attack(visual_manager, messages, args, epoch):
    ...
    for step in range(steps):
        ...
        loss1, loss2 = result["loss1"], result["loss2"]
        # Obtain the video hidden features
        if 'qwen' in args.model_name.lower():
            inputs.update({'hidden_states': outputs.hidden_states})
        hidden_states = visual_manager.get_vision_hidden_states(inputs)

        hidden_states = torch.stack(hidden_states, dim=0).squeeze().float().abs()
        hidden_states = hidden_states.reshape(-1, hidden_states.shape[-1])
        loss3 = -torch.norm(hidden_states, p='nuc')
...
def main(args):
    ...
    for epoch in range(start_epoch, len(datasets)):
        ...
        if is_video:
            ...
            if 'intern' in args.model_name.lower():
                messages[0]['content'].append({"type": "text", "text": f"What is the content of this image?"})
            else:
...
def get_args():
    ...
    parser.add_argument("--root_dir", type=str, default='save/baseline/VerboseVideo/')
