...
def get_special_ids(visual_manager, messages, segment_len, is_random=True):
    # ==========准备特殊字符===============
    suffix_len = segment_len * 2
    ...
    if is_random:
        ...
        while True:
            ...
            adv_suffix = tokenizer.decode((target_ids * suffix_len)[:suffix_len])
            if len(tokenizer.encode(adv_suffix, add_special_tokens=False)) == suffix_len:
                break
    else:
        # 贪心解码获得原始响应
        vision_tensors = visual_manager.get_vision_tensors(messages)
        output_ids, _, output_len = visual_manager.get_output_text(messages, vision_tensors,
                                                         max_new_tokens=100, greedy=True)
        # 计算每个候选种子的累积概率，选取最高的作为重复种子
        ...
        output_logits = logits[0, -1-output_len:-1, :]
        probs = F.log_softmax(output_logits, dim=-1)
        output_ids = output_ids[0, -output_len:]
        # 根据output_ids选取prob第二个维度
        probs = torch.gather(probs, 1, output_ids.unsqueeze(1)).squeeze()
        max_i = 0
        max_value = -100
        for i in range(len(probs)-segment_len):
            value = probs[i: i+segment_len].sum()
            if value > max_value:
                max_i = i
                max_value = value
        target_ids = output_ids[max_i:max_i+segment_len].unique().tolist()
